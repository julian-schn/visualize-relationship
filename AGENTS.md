...
5. Open `docs/DECISIONS.md` and record anything you had to decide that this spec left open.
...
docs/                  generated and hand-written docs (section 16)
...
and have a proposal filed the same run (section 13.3). Provisional entries render with a
warning badge in `docs/VOCAB.md`.
...
Warnings are written to `docs/STATS.md` on every build so they are visible rather than
buried in terminal output.
...
- Regenerate everything in `docs/` that is marked autogenerated
...
- **pre-commit**: run `validate`. Regenerate autogenerated docs (section 16) and `git add`
  them. Fail if validation fails.
...
in milestone 3, build verification in milestone 6, and docs checks in milestone 12. Before
then, hooks may use `npm run --if-present`; the commit that implements a gate removes the
corresponding conditional. Do not create no-op scripts merely to make a gate look green.

### 15.2 CI

Hooks are local and skippable, so `.github/workflows/check.yml` is the real enforcement:
on every push and PR, run `validate`, `test`, `build`, and `docs:check`.

`docs:check` regenerates the autogenerated docs and fails if the working tree is dirty
afterwards. That is what actually keeps documentation honest. A rule nobody checks is a
suggestion.
...
gate; after milestone 12, `validate`, `test`, `build`, and `docs:check` are all mandatory.

---

## 16. Documentation upkeep

Every file below is regenerated by `npm run docs`, called from the pre-commit hook and
checked in CI. Generated files start with:

```
<!-- AUTOGENERATED by npm run docs. Do not edit by hand. -->
```

Generators must be deterministic and idempotent: same input, byte-identical output, or
`docs:check` will thrash.

| File                | Source                          | Contents                                                     |
| ------------------- | ------------------------------- | ------------------------------------------------------------ |
| `docs/SCHEMA.md`    | `schema/*.json`                 | Human-readable field reference with examples                  |
| `docs/VOCAB.md`     | `vocab.json`                    | Every key, its label, symmetry, usage count, provisional flag |
| `docs/STATS.md`     | the data                        | Counts, generation span, component count, open warnings       |
| `docs/KINSHIP.md`   | `src/kinship/terms.*`           | The full derivation table per language                        |
| `CHANGELOG.md`      | conventional commits            | Grouped by type since the last tag                            |

Hand-written and kept current by the agent, not generated:

- `README.md`: what this is, how to build it, how to add a person. Under 60 lines.
- `docs/DECISIONS.md`: short dated entries for every non-obvious choice made.
- `AGENTS.md`: this file. **If you change how the system works, change this file in the same
  commit.** A spec that lags the code is worse than no spec.
...
12. **Docs generators** and the `docs:check` gate.
13. **Polish**: keyboard shortcuts, exports, dark mode, reduced motion.
